import { db, doc, getDoc, updateDoc, deleteDoc, SCHEMA, collection, query, where, getDocs, limit } from '/js/services/firebase.js';
...
let videoUrlInput, titleInput, posterPreview, selectedPosterUrlInput, thumbnailOptionsContainer, movieId;
...
async function handleSmartFetch(url) { 
    ...
    if (result.exists && (result.type !== 'movie' || result.data?.id !== movieId)) {
        UI.showToast(`ตรวจพบลิงก์นี้ใน${result.type === 'movie' ? 'ภาพยนตร์' : 'ซีรีส์'}เรื่องอื่นแล้ว!`, 'warning');
...
async function isDuplicateContent(videoUrl) {
    ...
    if (result.exists) {
        ...
        const q = query(collection(db, SCHEMA.COLLECTIONS.MOVIES), where('videoUrl', '==', videoUrl), limit(5));
