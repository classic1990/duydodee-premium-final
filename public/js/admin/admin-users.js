import { db, collection, getDocs, doc, updateDoc, query, orderBy, limit, startAfter, getCountFromServer, SCHEMA } from '/js/services/firebase.js';
            ...
            <tr class="group hover:bg-white/[0.02] transition-all border-b border-white/5">
                ...
                <td class="p-6 text-right">
                    <div class="flex items-center justify-end gap-3">
                        <button onclick="window.openEditModal('${user.uid}')" class="w-9 h-9 rounded-xl bg-blue-500/10 text-blue-500 border border-blue-500/20 flex items-center justify-center hover:bg-blue-500 hover:text-white transition-all haptic-btn" title="แก้ไขสิทธิ์">
                            <i data-lucide="edit-3" class="w-4 h-4"></i>
                        </button>
                        <button onclick="window.toggleBan('${user.uid}', ${!!user.isBanned})" class="w-9 h-9 rounded-xl bg-red-500/10 text-red-500 border border-red-500/20 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all haptic-btn" title="${user.isBanned ? 'ปลดระงับ' : 'ระงับสิทธิ์'}">
