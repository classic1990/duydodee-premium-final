...
export const UI = {
    ...
    initNavbar: () => {
        ...
        window.addEventListener('scroll', () => {
            if (window.scrollY > 50) { nav.classList.add('nav-glass', 'py-3'); nav.classList.remove('py-6'); }
            else { nav.classList.remove('nav-glass', 'py-3'); nav.classList.add('py-6'); }
        });
    ...
    initAuthStatus: () => {
        ...
        AuthService.onStateChanged(async (user) => {
            if (user) {
                ...
                if (dArea) dArea.innerHTML = '<a href="/login.html" class="btn-primary py-2 px-6 text-[10px]">เข้าสู่ระบบ</a>';
            }
            UI.refreshIcons();
        });
    },
    ...
    highlightActiveNav: () => {
        ...
        document.querySelectorAll('.nav-link').forEach(link => {
            if (link.getAttribute('href') === path) link.classList.add('text-brand-primary');
        });
    },
    ...
    renderVIPUpgradeModal: async () => {
        ...
        try {
            ...
            document.getElementById('submit-payment').onclick = async () => {
                ...
                if(!name || !amount || !time) return UI.showToast('กรุณากรอกให้ครบ', 'error');
