import { db, collection, getDocs, query, orderBy, limit, SCHEMA, getCountFromServer, getAggregateFromServer, sum, where, doc, updateDoc, getDoc } from '../services/firebase.js';
...
async function initPaymentSettings() {
    try {
        ...
        if (snap.exists()) {
            ...
            document.getElementById('dash-wallet-number').value = data.wallet || '';
            document.getElementById('dash-account-name').value = data.name || '';
        }
    } catch (e) {
...
function setupPaymentForm() {
    const form = document.getElementById('dashboard-payment-form');
    if (!form) return;

    form.onsubmit = async (e) => {
        e.preventDefault();
        const wallet = document.getElementById('dash-wallet-number').value;
        const name = document.getElementById('dash-account-name').value;
