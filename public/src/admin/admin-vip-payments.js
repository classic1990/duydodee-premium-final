...
function renderTable(payments) {
  ...
  if (payments.length === 0) {
    paymentList.innerHTML =
      '<tr><td colspan="6" class="p-20 text-center text-xs text-gray-500 Thai-font opacity-40">ไม่พบรายการชำระเงินในระบบ</td></tr>';
    return;
  }
...
window.verifyPayment = async (id, userId) => {
  ...
  try {
    ...
    if (userId && userId !== 'guest') {
      ...
      if (
        currentRole === SCHEMA.ROLES.MASTER.toLowerCase() ||
        currentRole === SCHEMA.ROLES.ADMIN.toLowerCase() ||
        currentRole === 'master'
      ) {
