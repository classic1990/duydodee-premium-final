  ...
  try {
    ...
    batch.update(doc(db, SCHEMA.COLLECTIONS.SERIES, seriesId), {
      ...
      poster:
        ...
        `https://img.youtube.com/vi/${UI.extractYouTubeId(episodes[0].videoUrl)}/maxresdefault.jpg`,
      episodeCount: episodes.length,
      updatedAt: serverTimestamp()
    });

    const oldEpSnap = await getDocs(
      collection(db, SCHEMA.COLLECTIONS.SERIES, seriesId, 'episodes')
    );
    oldEpSnap.forEach((d) => batch.delete(d.ref));

    episodes.forEach((ep) => {
      batch.set(doc(collection(db, SCHEMA.COLLECTIONS.SERIES, seriesId, 'episodes')), {
        ...ep,
