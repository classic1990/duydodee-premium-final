import { db, collection, addDoc, serverTimestamp, doc, getDocs, query, where, limit, writeBatch, SCHEMA, logActivity } from '/src/services/firebase.js';
 ...
 * Unified Logic for High-Impact Series Onboarding
 */
...
function initForm() {
    ...
    // Add first episode by default
    addEpisode();

    if (addEpBtn) {
        addEpBtn.addEventListener('click', () => addEpisode());
    }
...
function addEpisode(title = '', url = '') {
    ...
    if (urlInput) {
        urlInput.addEventListener('input', debounce(async () => {
            ...
            if (isFirst && val) fetchSeriesInfo(val);
        }, 600));
    }
}
...
async function handleAddSeries(e) {
    ...
    try {
        ...
        // 2. Add Episodes to Series subcollection AND add episodes as array on the series document itself for easier client-side fetch
        episodes.forEach(ep => {
            const epRef = doc(collection(db, SCHEMA.COLLECTIONS.SERIES, seriesRef.id, 'episodes'));
            batch.set(epRef, { ...ep, createdAt: serverTimestamp() });
        });
        
        // Also update series document with array of episodes
        ...
        await logActivity('ADD_SERIES', `เพิ่มซีรีส์ใหม่: ${title} (${episodes.length} ตอน)`);
