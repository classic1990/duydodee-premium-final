...
export const UI = {
  ...
  setFallbackImage(imgElement) {
    if (imgElement) {
      imgElement.src = '/assets/logo/DUYDODEE.png';
    }
  },
  ...
  async loadImageWithCache(url, imgElement) {
    ...
    imgElement.onerror = () => this.setFallbackImage(imgElement);

    try {
      const response = await fetch(url);
      if (response.status === 429) {
        console.warn('Rate limited, using placeholder');
        this.setFallbackImage(imgElement);
        return;
      }
  ...
  renderiPhonePlayer: (data, episodes = [], activeIndex = 0, isSeries = false) => {
    ...
    const isVertical =
      data.category && (data.category.includes('แนวตั้ง') || data.category.includes('Vertical'));
