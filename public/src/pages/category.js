...
async function initCategoryPage() {
  ...
  if (sortSelector) {
    sortSelector.onchange = (e) => loadCategoryContent(activeCol, activeCat, e.target.value, false);
  }

  const loadMoreBtn = document.getElementById('load-more-btn');
  if (loadMoreBtn) {
    loadMoreBtn.onclick = () =>
      loadCategoryContent(activeCol, activeCat, sortSelector?.value || 'createdAt', true);
  }
...
async function loadCategoryContent(
  ...
  try {
    ...
    const isVerticalSearch =
      Array.isArray(categoryInput) &&
      categoryInput.some((c) => c && typeof c === 'string' && c.includes('แนวตั้ง'));
    ...
    if (empty && !isAppend) {
      ...
      if (emptyState) {
        ...
        emptyState.innerHTML =
          '<div class="col-span-full py-20 text-center Thai-font opacity-40">ไม่พบเนื้อหาในหมวดหมู่ที่คุณต้องการ</div>';
      }
      if (pagContainer) {
        pagContainer.classList.add('hidden');
      }
