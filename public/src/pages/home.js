...
function renderSlider(slides, container) {
    ...
    showSlide(currentIndex); if (slides.length > 1) {
        resetTimer();
    }
}
...
async function loadDiscoveryContent(reset = false) {
    ...
    try {
        ...
        // ปรับ Logic ให้ตรวจเช็คจากจำนวน items ที่ได้มา
        isEndOfContent = empty || (items.length === 0);
        
        // หาก items ที่ได้มาน้อยกว่า pageSize ก็ถือว่าเป็นหน้าสุดท้าย แต่ให้โหลดได้
        const hasMore = items.length === UI_CONFIG.PAGE_SIZE;
...
function updatePaginationUI() {
    ...
    if (!prevPageBtn || !nextPageBtn || !pageNumbersEl) {
        return;
    }
